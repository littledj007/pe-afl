...
class PE(object):
    ...
    def write(self, filename=None, append=None, cut=None):
        """Write the PE file.
        ...
        # used by pe-afl
        # used for cut the overlays
        if cut:
            file_data = file_data[:-cut]
        # used for append new section
        if append:
            file_data += append
            ...
            if not idx+export_dir.Base in ordinals:
                ...
                exports.append(
                    ExportData(
                        ...
                        address = symbol_address,
                        address_offset = self.get_offset_from_rva(export_dir.AddressOfFunctions+idx*4),
                        name = None,
                        forwarder = forwarder_str))
    ...
    def get_section_by_name(self, name):
        """Get the section containing the given name."""

        sections = [s for s in self.sections if name in s.Name]
    ...
    def get_directory_by_name(self, name):
        """Get the directory containing the given name."""

        d = [s for s in self.OPTIONAL_HEADER.DATA_DIRECTORY if name in s.name]
